...
# Dataset-neutral SemDeDup quality A/B runner for an 8x B200-style NanoChat setup.
#
# Modes:
#   bash runs/semdedup_quality_b200.sh baseline
#   bash runs/semdedup_quality_b200.sh semdedup
#   bash runs/semdedup_quality_b200.sh randomdrop
#   bash runs/semdedup_quality_b200.sh both
#
...
SEMD_MODEL="${SEMD_MODEL_IDENTIFIER:-${SEMD_MODEL:-google/embeddinggemma-300m}}"
...
SEMD_STAGED_DOCS_PER_FILE="${SEMD_STAGED_DOCS_PER_FILE:-}"
SEMD_INPUT_FILES_PER_PARTITION="${SEMD_INPUT_FILES_PER_PARTITION:-}"
SEMD_EMBEDDING_DIM="${SEMD_EMBEDDING_DIM:-}"
SEMD_EMBEDDING_MAX_CHARS="${SEMD_EMBEDDING_MAX_CHARS:-}"
SEMD_KMEANS_FILES_PER_GROUP="${SEMD_KMEANS_FILES_PER_GROUP:-}"
...
SEMD_REUSE_STAGED_INPUTS="${SEMD_REUSE_STAGED_INPUTS:-0}"
...
export SEMD_BACKEND SEMD_MODEL SEMD_MODEL_IDENTIFIER SEMD_EPS SEMD_N_CLUSTERS SEMD_DISTANCE_METRIC SEMD_WHICH_TO_KEEP
export SEMD_PAIRWISE_BATCH_SIZE SEMD_STAGED_DOCS_PER_FILE SEMD_INPUT_FILES_PER_PARTITION SEMD_EMBEDDING_DIM
export SEMD_EMBEDDING_MAX_CHARS SEMD_KMEANS_FILES_PER_GROUP SEMD_VLLM_INIT_KWARGS_JSON
...
keys = [
    ...
    "SEMD_BACKEND", "SEMD_MODEL", "SEMD_MODEL_IDENTIFIER", "SEMD_EPS", "SEMD_N_CLUSTERS",
    "SEMD_DISTANCE_METRIC", "SEMD_WHICH_TO_KEEP", "SEMD_PAIRWISE_BATCH_SIZE",
    "SEMD_STAGED_DOCS_PER_FILE", "SEMD_INPUT_FILES_PER_PARTITION", "SEMD_EMBEDDING_DIM",
    "SEMD_EMBEDDING_MAX_CHARS", "SEMD_KMEANS_FILES_PER_GROUP", "SEMD_VLLM_INIT_KWARGS_JSON",
...
run_one() {
    ...
    if [[ "$run_kind" == "semdedup" ]]; then
        if [[ "$DO_SEMDEDUP" == "1" ]]; then
            ...
            if [[ -n "$SEMD_STAGED_DOCS_PER_FILE" ]]; then
                SEMD_ARGS+=(--staged-docs-per-file "$SEMD_STAGED_DOCS_PER_FILE")
            fi
            if [[ -n "$SEMD_INPUT_FILES_PER_PARTITION" ]]; then
                SEMD_ARGS+=(--input-files-per-partition "$SEMD_INPUT_FILES_PER_PARTITION")
            fi
            if [[ -n "$SEMD_EMBEDDING_DIM" ]]; then
                SEMD_ARGS+=(--embedding-dim "$SEMD_EMBEDDING_DIM")
            fi
            if [[ -n "$SEMD_EMBEDDING_MAX_CHARS" ]]; then
                SEMD_ARGS+=(--embedding-max-chars "$SEMD_EMBEDDING_MAX_CHARS")
            fi
            if [[ -n "$SEMD_KMEANS_FILES_PER_GROUP" ]]; then
                SEMD_ARGS+=(--kmeans-files-per-group "$SEMD_KMEANS_FILES_PER_GROUP")
            fi
            ...
            if [[ "$SEMD_REUSE_STAGED_INPUTS" == "1" ]]; then
                SEMD_ARGS+=(--reuse-staged-inputs)
            fi
            ...
            "$PYTHON_BIN" -m scripts.build_semdedup_dataset "${SEMD_ARGS[@]}" 2>&1 | tee "$run_dir/semdedup.log"
        else
            if [[ ! -d "$data_dir" ]]; then
    ...
    if [[ "$run_kind" == "randomdrop" ]]; then
        if [[ "$DO_RANDOM_DROP" == "1" ]]; then
            ...
            "$PYTHON_BIN" -m scripts.build_random_drop_dataset "${RANDOM_DROP_ARGS[@]}" 2>&1 | tee "$run_dir/randomdrop.log"
        else
            if [[ ! -d "$data_dir" ]]; then
    ...
    if [[ -n "$stats_source" ]]; then
        ...
        "$PYTHON_BIN" -m scripts.parquet_data_stats "${STATS_ARGS[@]}"
    fi
