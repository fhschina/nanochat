...
MODE="${1:-all}"
if [[ "$MODE" != "smoke" && "$MODE" != "pilot-baseline" && "$MODE" != "pilot-semdedup" && "$MODE" != "pilot" && "$MODE" != "formal" && "$MODE" != "report" && "$MODE" != "resume" && "$MODE" != "all" ]]; then
    echo "Usage: $0 smoke|pilot-baseline|pilot-semdedup|pilot|formal|report|resume|all" >&2
    exit 2
fi
